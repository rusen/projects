...
InstanceList *FindInstances(Graph *g1, Graph *g2, Parameters *parameters)
{
   ...
   BOOLEAN noMatches;
   BOOLEAN found;
   double threshold = parameters->threshold * (g1->numEdges + g1->numVertices)
		   + DOUBLE_EQUALITY_EPSILON;	// Added a very small value for extra safety.
...
//   printf("****************************\n");
//   PrintLabelList(parameters->labelList);
//   printf("****************************\n");
   instanceList = FindSingleVertexInstances(g2, vertex1, parameters, threshold);
   ...
   while ((vertex1 != NULL) && (! noMatches)) 
   {
      ...
      for (e1 = 0; ((e1 < vertex1->numEdges) && (! noMatches)); e1++) 
      {
         ...
         if (! edge1->used) 
         {
            ...
            instanceList =
               ExtendInstancesByEdge(instanceList, g1, edge1, g2, parameters, threshold);
...
//   PrintInstanceList(instanceList, parameters->posGraph, parameters->labelList);
//   fflush(stdout);
   return instanceList;
}
...
//         (Parameters *parameters)
//		   (double threshold) - overall elasticity allowed
//
                                        ...
                                        Parameters *parameters, double threshold)
{
   ...
   {
	  matchCost = LabelMatchFactor(graph->vertices[v].label, vertex->label, parameters->labelList);
                                    ...
                                    double threshold)
{
   ...
   while (instanceListNode != NULL) 
   {
      ...
      allowedCost = threshold - instance->minMatchCost;
      ...
      for (v2 = 0; v2 < instance->numVertices; v2++) 
      {
         ...
         for (e2 = 0; e2 < vertex2->numEdges; e2++) 
         {
            ...
            if ((! edge2->used))
			{
            	// estimate edge match cost, proceed if it is smaller than given threshold
				edgeMatchCost = EdgesMatch(g1, edge1, g2, edge2, parameters, threshold);
				if (edgeMatchCost != -1 && edgeMatchCost < allowedCost)
				{
				   // add new instance to list
				   newInstance =
					  CreateExtendedInstance(instance, instance->vertices[v2],
											 vertex2->edges[e2], g2);
				   // add cost to the new instance
				   newInstance->minMatchCost = instance->minMatchCost + edgeMatchCost;
...
//         (double threshold) - overall elasticity allowed
//
// RETURN: (double) - Match cost if the edges are matched, -1 otw.
//
...
double EdgesMatch(Graph *g1, Edge *edge1, Graph *g2, Edge *edge2,
                   Parameters *parameters, double threshold)
{
   ULONG vertex11Label;
   ULONG vertex12Label;
   ...
   ULONG vertex22Label;
   double matchCost;
   double edgeMatchCost1, edgeMatchCost2=threshold+1;	// assign edgeMatchCost2 an invalid value at first.
   ...
   /*
   printf("****** EDGE LABELS ********\n");
   PrintLabel(vertex11Label, parameters->labelList, stdout);
   printf("***");
   PrintLabel(vertex12Label, parameters->labelList, stdout);
   printf("***");
   PrintLabel(vertex21Label, parameters->labelList, stdout);
   printf("***");
   PrintLabel(vertex22Label, parameters->labelList, stdout);
   printf("***");
   PrintLabel(edge1->label, parameters->labelList, stdout);
   printf("***");
   PrintLabel(edge2->label, parameters->labelList, stdout);
   printf("****** EDGE LABELS ********\n");
*/
   matchCost = LabelMatchFactor(edge1->label, edge2->label, parameters->labelList);

   if (matchCost <= threshold &&
       (edge1->directed == edge2->directed)) 
   {
	   // Estimate cost of matching vertex labels at both ends,
	   // and in the reverse direction if edges are undirected.
	   // Get the minimum of two values.
	   edgeMatchCost1 = LabelMatchFactor(vertex11Label, vertex21Label, parameters->labelList) +
			   LabelMatchFactor(vertex12Label, vertex22Label, parameters->labelList);
	   if (! edge1->directed)
		   edgeMatchCost2 = LabelMatchFactor(vertex11Label, vertex22Label, parameters->labelList) +
		   			   LabelMatchFactor(vertex12Label, vertex21Label, parameters->labelList);
	   if (edgeMatchCost1>edgeMatchCost2)
		   edgeMatchCost1 = edgeMatchCost2;

	   // Add the vertex label matching cost to edge label matching cost.
	   matchCost += edgeMatchCost1;

	  // Return cost if smaller than given threshold
      if (matchCost <= threshold)
      {
 //   	   printf("%lf\n", matchCost);
         return matchCost;
         ...
         if (instanceListNode->instance != NULL) 
         {
                ...
                (! InstanceListOverlap(instance, newInstanceList))) 
            {
               ...
               if (GraphMatch(subGraph, instanceGraph, parameters->labelList,
                              thresholdLimit, & matchCost, NULL, FALSE))
               {
