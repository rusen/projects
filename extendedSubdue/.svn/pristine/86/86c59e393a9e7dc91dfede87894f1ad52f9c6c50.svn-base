...
//                                 ignored if NULL
// 			(BOOLEAN inexactLabelsOnly) - TRUE if only inexact matching of
// 				labels are allowed ( not structural changes like vertex
// 				addition/deletion). FALSE otherwise.
//
...
BOOLEAN GraphMatch(Graph *g1, Graph *g2, LabelList *labelList,
                   double threshold, double *matchCost,
                   VertexMap *mapping, BOOLEAN inexactLabelsOnly)
{
   double cost;
   ...
   if (g1->numVertices < g2->numVertices)
      cost = InexactGraphMatch(g2, g1, labelList, threshold, mapping, inexactLabelsOnly);
   else 
      cost = InexactGraphMatch(g1, g2, labelList, threshold, mapping, inexactLabelsOnly);
...
// is not considered.  Graph g1 should be the larger graph in terms of
                         ...
                         BOOLEAN inexactLabelsOnly)
{
   ...
   double cost = 0.0;
   double newCost = 0.0;
   double labelMatchCost = 0.0;
   double labelBasedCost = 0.0;
   double labelBasedThreshold = 0.0;
   ULONG numNodes = 0;
   ...
   // If only inexact label matching is allowed, we set threshold 0.0,
   // with special treatment so that label substitution with costs in the
   // range of [0,1) will not count to it.
   if (inexactLabelsOnly)
   {
	   labelBasedThreshold = threshold;
	   threshold = 0.0;
   }
         ...
         { // expand node's partial mapping
           // compute mappings between graphs' vertices
            ...
            if ((newCost <= threshold) && (newCost < bestNode.cost) &&
          		  (!inexactLabelsOnly || labelBasedCost <= labelBasedThreshold))
            {
            ...
            for (v2 = 0; v2 < nv2; v2++) 
            {
               if (mapped2[v2] == VERTEX_UNMAPPED) 
               {
                  ...
                  mapped2[v2] = v1;
                  newCost = node.cost;
                  labelMatchCost = SUBSTITUTE_VERTEX_LABEL_COST *
                                       LabelMatchFactor(g1->vertices[v1].label,
                                                        g2->vertices[v2].label, labelList);

                  // If only inexact labels are to be matched, modify label based cost.
                  if (labelMatchCost < SUBSTITUTE_VERTEX_LABEL_COST && inexactLabelsOnly)
                  {
                	  labelBasedCost =+ labelMatchCost;
                  }
                  else
                  {
                	  newCost += labelMatchCost;
                  }
