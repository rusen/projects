...
// TODO: Will be tested for correctness.
//---------------------------------------------------------------------------
...
void PruneGraph(Graph *g, char *labelStr, LabelList *labelList)
{
   ...
   strncpy(buffer, labelStr, TOKEN_LEN);

   if (*endptr != '\0')
   {
	   // Read string label.
      label.labelType = STRING_LABEL;
      label.labelValue.stringLabel = strtok(labelStr, " ");

      if (*(buffer + strlen(label.labelValue.stringLabel)) == ' ')
      {
		   // Read vector label's string value and other fields.
		  label.labelType = VECTOR_LABEL;
		  label.labelValue.vStruct.stringLabel = label.labelValue.stringLabel;
		  label.labelValue.vStruct.vectComparisonType = (UCHAR) strtod(labelStr, &endptr);
