...
void SubListInsert(Substructure *sub, SubList *subList, ULONG max,
                   BOOLEAN valueBased, LabelList *labelList, Parameters *parameters)
{
   SubListNode *subIndex = NULL;
   SubListNode *parentSubIndex = NULL;
   SubListNode *deletedSubIndex = NULL;
   SubListNode *subIndexPrevious = NULL;
   SubListNode *newSubListNode = NULL;
   ULONG numSubs = 0;
   ULONG numDiffVals = 0;
   BOOLEAN inserted = FALSE;
   BOOLEAN forceDiversity = parameters->forceDiversity;
   ...
   while ((subIndex != NULL) && (subIndex->sub->value >= sub->value))
   {
	  if (!forceDiversity)
	  {
		  // Regular SUBDUE behaviour. We check for exact
		  // matching in sub-list membership testing.
		  if (subIndex->sub->value == sub->value)
		  {
			 if (GraphMatch(subIndex->sub->definition, sub->definition,
				 labelList, 0.0, NULL, NULL, FALSE))
			 {
	  ...
	  {
		  // If diversity is forced, we will exclude subs that match
		  // a higher-value sub in the list, even though in an inexact manner.
		 threshold *= (sub->definition->numVertices + sub->definition->numEdges);
//
//		 printf("\n-------- matching two subs ---------\n");
//		 PrintSub(subIndex->sub, parameters);
//		 PrintSub(sub, parameters);
//		 printf("\n-------- matching two subs ---------\n");
//		 fflush(stdout);

		 if (GraphMatch(subIndex->sub->definition, sub->definition,
			 labelList, threshold, NULL, NULL, TRUE))
			 // TODO: Maybe use return match cost to determine quality of sub?
		 {
			 FreeSubListNode(newSubListNode);
			 return;
		 }
	  ...
	  parentSubIndex = subIndex;
	  subIndex = subIndex->next;
   }

   // Remove subs with lower scores that match the current sub,
   // if diversity is forced.
   if (forceDiversity)
   {
	   while(subIndex != NULL){
			 threshold *= (sub->definition->numVertices + sub->definition->numEdges);
			 // We only allow substitution of labels of the same kind,
			 // therefore retaining the structural integrity of the sub
			 // in matching process.
			 if (GraphMatch(subIndex->sub->definition, sub->definition,
				 labelList, threshold, NULL, NULL, TRUE))
			 {
...
//				 printf("******END*******\n");
				 // Delete subIndex
				 if (subIndex == subList->head)
				 {
					 subList->head = subList->head->next;
				 }
				 else
				 {
					 parentSubIndex->next = subIndex->next;
				 }
				 deletedSubIndex = subIndex;
				 subIndex = subIndex->next;
				 FreeSubListNode(deletedSubIndex);
			 }
			 else
			 {
				 parentSubIndex = subIndex;
   ...
   // if subList became empty, insert new sub and exit
  if (subList->head == NULL)
  {
	 subList->head = newSubListNode;
	 return;
...
// RETURN:  void
//
// PURPOSE: Print given list of substructures to output file.
//---------------------------------------------------------------------------

void PrintSubListToFile(SubList *subList, Parameters *parameters)
{
   ULONG counter = 1;
   SubListNode *subListNode = NULL;
   ...
   if (outputFile == NULL)
   {
	   fprintf(stderr, "Output file cannot be opened. Disabling file output..\n");
   ...
   if (subList != NULL)
   {
      ...
      while (subListNode != NULL)
      {
		   // Form and print sub name.
    	   if (parameters->incremental)
		   {
				 sprintf(subLabelString, "Label: %s_%lu_%lu\n", SUB_LABEL_STRING, counter,
						 GetCurrentIncrementNum(parameters));
		   }
		   else
		   {
//			   	 sprintf(subLabelString, "PS\n");
				 sprintf(subLabelString, "Label: %s_%lu\n", SUB_LABEL_STRING, counter);
		   }
    	   fprintf(outputFile, subLabelString);
		   fprintf(outputFile, "\nDefinition:\n");

		   // Print substructure definition to the file.
		   WriteGraphToFile(outputFile, subListNode->sub->definition, parameters->labelList,
		                       0, 0, subListNode->sub->definition->numVertices, FALSE);
		   fprintf(outputFile, "\n");

			// Print instances to the file. (pos/neg)

   		 if (subListNode->sub->instances != NULL)
   			 PrintInstanceListToFile(subListNode->sub->instances, parameters->posGraph,
   					 parameters->labelList, outputFile,
   					 threshold * (subListNode->sub->definition->numVertices +
   							 subListNode->sub->definition->numEdges));
   		 if (subListNode->sub->negInstances != NULL)
   			 PrintInstanceListToFile(subListNode->sub->negInstances, parameters->negGraph,
   					 parameters->labelList, outputFile,
   					 threshold * (subListNode->sub->definition->numVertices +
   							 subListNode->sub->definition->numEdges));
...
void PrintSub(Substructure *sub, Parameters *parameters)
{
   ...
   if (sub != NULL) 
   {
      ...
      {
         printf("    re ");
         PrintLabel(sub->recursiveEdgeLabel, labelList, stdout);
         printf("\n");
      }
...
void PrintInstance(Instance *instance, ULONG vertexOffset, Graph *graph, LabelList *labelList)
{
   ...
   if (instance != NULL) 
   {
      ...
      {
         printf("    ");
         PrintVertex(graph, instance->vertices[i], vertexOffset, labelList, stdout);
      }
      for (i = 0; i < instance->numEdges; i++) 
      {
         printf("    ");
         PrintEdge(graph, instance->edges[i], vertexOffset, labelList, stdout);
      }
   }
}

//---------------------------------------------------------------------------
// NAME: PrintInstanceToFile
//
// INPUTS: (Instance *instance) - instance to print
//         (ULONG vertexOffset) - offset for vertex index for this instance
//         (Graph *graph) - graph containing instance
...
// RETURN: (void)
//
// PURPOSE: Print given instance to file.
//---------------------------------------------------------------------------

void PrintInstanceToFile(Instance *instance, ULONG vertexOffset, Graph *graph,
						LabelList *labelList, FILE *outputFile)
{
   ULONG i;
   ...
   if (instance != NULL)
   {
      for (i = 0; i < instance->numVertices; i++)
      {
         PrintVertex(graph, instance->vertices[i], vertexOffset, labelList, outputFile);
      }
      for (i = 0; i < instance->numEdges; i++)
      {
         PrintEdge(graph, instance->edges[i], vertexOffset, labelList, outputFile);
      }
   }
}
                        ...
                        LabelList *labelList)
{
   ...
   if (instanceList != NULL) 
   {
      ...
      while (instanceListNode != NULL) 
      {
         ...
         printf("\nConfidence: %lf", instanceListNode->instance->minMatchCost);
         instanceListNode = instanceListNode->next;
         i++;
      }
   }
...
//---------------------------------------------------------------------------
// NAME: PrintInstanceListToFile
//
// INPUTS: (InstanceList *instanceList) - list of instances
//         (Graph *graph) - graph containing instances
//         (LabelList *labelList) - labels used in input graph
// 		   (FILE *outputFile) - the output file
// 		   (double threshold) - maximum cost allowed for matching two graphs
//
// RETURN: (void)
//
// PURPOSE: Print array of instances to file.
//---------------------------------------------------------------------------

void PrintInstanceListToFile(InstanceList *instanceList, Graph *graph,
                        LabelList *labelList, FILE * outputFile, double threshold)
{
   ULONG i = 0;
   InstanceListNode *instanceListNode;
   ...
   if (instanceList != NULL)
   {
      ...
      while (instanceListNode != NULL)
      {
         fprintf(outputFile, "  Instance %lu:\n", i + 1);
         PrintInstanceToFile(instanceListNode->instance, 0, graph, labelList, outputFile);
         fprintf(outputFile, "Confidence: %lf\n\n", instanceListNode->instance->minMatchCost);
 //        fprintf(outputFile, "Confidence: %lf\n",
 //               		 instanceListNode->instance->minMatchCost);
                     ...
                     double threshold, double *cost)
{
   ...
   if (GraphMatch(g1, g2, labelList, threshold, NULL, subMapping, FALSE))
   {
