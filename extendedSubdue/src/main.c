...
int main (int argc, char **argv)
{
   ...
   else
   {
      ...
      // If discovery level is 1, we stop here.
      if (parameters->discoveryLevel == 1){
    	  done = TRUE;
      }
      ...
      while ((iteration <= parameters->iterations) && (!done))
      {
         ...
         else
         {
            ...
            if (parameters->outputToFile && parameters->fileOutputLevel == 1)
            {
            ...
            // In the ultimate file output level, we print the label and instances of each sub.
            // TODO: ADD DESCRIPTION HERE.
            if (parameters->outputToFile && parameters->fileOutputLevel == 3)
            {
            	SubListNode *currEntry = subList->head;
                ...
                PrintSubListToFile(subList, parameters);
/*
                int subCounter = 1;
            	...
            	while (currEntry != NULL)
            	{
            		...
            		sprintf(subLabelString, "%s_%i", SUB_LABEL_STRING, subCounter);
					fprintf(outputFile, "Label: %s\n", subLabelString);

					// Print instances
            		PrintInstanceListToFile(currEntry->sub->instances, parameters->posGraph,
       					 parameters->labelList, outputFile,
       					 parameters->threshold * (subList->head->sub->definition->numVertices +
       							 subList->head->sub->definition->numEdges));
...
Parameters *GetParameters(int argc, char *argv[])
{
   ...
   parameters->discoveryLevel = 3;
   parameters->forceDiversity = FALSE;
   ...
   parameters->compressWithAll = FALSE;
   if (argc < 2)
   {
   ...
   while (i < (argc - 1))
   {
      ...
      else if (strcmp(argv[i], "-compressAll") == 0)
      {
         parameters->compressWithAll = TRUE;
      }
      else if (strcmp(argv[i], "-discovery") == 0)
      {
    	  i++;
    	  sscanf(argv[i], "%lu", &ulongArg);
    	  if ((ulongArg < 1) || (ulongArg > 3))
    	  {
    		  fprintf(stderr, "%s: discovery must be 1-3\n", argv[0]);
    		  exit(1);
    	  }
    	  parameters->discoveryLevel = ulongArg;
      }
      ...
      else if (strcmp(argv[i], "-diverse") == 0)
      {
         parameters->forceDiversity = TRUE;
      }
      else if (strcmp(argv[i], "-fileoutput") == 0)
      {
    	  i++;
    	  sscanf(argv[i], "%lu", &ulongArg);
    	  if ((ulongArg < 1) || (ulongArg > 3))
    	  {
			  fprintf(stderr, "%s: file output must be 1-3\n", argv[0]);
			  exit(1);
		  }
    	  parameters->fileOutputLevel = ulongArg;
      }
