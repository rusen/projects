...
//
// PURPOSE: Reads a label (string, numeric or vector) from the given file and
// stores the label in the given label list if not already there.
// Returns the label's index in the label list.
// Note that a vector label also has a string label associated with it.
//---------------------------------------------------------------------------

ULONG ReadLabel(FILE *fp, LabelList *labelList, ULONG *pLineNo)
{
   char token[TOKEN_LEN];
   char tempToken[TOKEN_LEN];
   char *endptr;
   char tempCh;
   Label label;
   int temp, vectorItr;
   ULONG retLabel, lineNo;
   long int pos;
   ...
   label.labelValue.numericLabel = temp;
   ...
   if (*endptr != '\0')
   {
      ...
      // If we did not get to the next line, this is a vector label.
      if (*pLineNo == lineNo)
      {
    	  ReadToken(tempToken, fp, pLineNo);
		  temp = strtod(tempToken, &endptr);

		  label.labelType = VECTOR_LABEL;
		  label.labelValue.vStruct.stringLabel = token;
		  label.labelValue.vStruct.vectComparisonType = (UCHAR) temp;

		  // Read number of dimensions and each feature into the vector label.
		  label.labelValue.vStruct.numberOfDimensions = (int) strtod(endptr+1, &endptr);
		  label.labelValue.vStruct.vectorLabel = (double *) malloc((int)label.labelValue.vStruct.numberOfDimensions * sizeof(double));

		  double mean = 0, stdev = 0;	// Heuristics that will be used to compare two labels.

		  // Read the label into this vector.
		  for ( vectorItr = 0; *endptr != '\0'; vectorItr++) {
			  label.labelValue.vStruct.vectorLabel[vectorItr] = strtod(endptr+1, &endptr);
			  mean += label.labelValue.vStruct.vectorLabel[vectorItr];
		  }
		  mean /= label.labelValue.vStruct.numberOfDimensions;

		  // Calculate stdev.
		  for ( vectorItr = 0; vectorItr < label.labelValue.vStruct.numberOfDimensions; vectorItr++){
			  stdev += pow((label.labelValue.vStruct.vectorLabel[vectorItr] - mean), 2);
		  }
		  stdev = sqrt(stdev / label.labelValue.vStruct.numberOfDimensions);
      }
   }

   retLabel = StoreLabel(&label, labelList);

   // Free the structures in temporary label
   if (label.labelType == VECTOR_LABEL)
   {
	   free(label.labelValue.vStruct.vectorLabel);
   }
   return retLabel;
}
...
void PrintGraph(Graph *graph, LabelList *labelList)
{
   ...
   if (graph != NULL) 
   {
      ...
      {
         printf("    ");
         PrintVertex(graph, v, 0, labelList, stdout);
      }
      // print edges
      for (e = 0; e < graph->numEdges; e++) 
      {
         printf("    ");
         PrintEdge(graph, e, 0, labelList, stdout);
      }
   }
}
...
// 		   (FILE *outputFile) - output file handle, prints to stdout if null.
//
// RETURN: (void)
//
		...
		FILE *outputFile)
{
   if (outputFile == NULL)
	   outputFile = stdout;
   fprintf(outputFile, "v %lu ", vertexIndex - vertexOffset + 1);
   PrintLabel(graph->vertices[vertexIndex].label, labelList, outputFile);
   fprintf(outputFile, "\n");
}
...
void PrintEdge(Graph *graph, ULONG edgeIndex, ULONG vertexOffset,
					LabelList *labelList, FILE *outputFile)
{
   ...
   fprintf(outputFile, " %lu %lu ", edge->vertex1 - vertexOffset + 1, edge->vertex2 - vertexOffset + 1);
   PrintLabel(edge->label, labelList, outputFile);
   fprintf(outputFile, "\n");
}
