...
#define VECTOR_LABEL 2

// Distance types for vector labels
#define DIST_L1 1
#define DIST_L2 2
#define DIST_COS 3
#define DIST_CORR 4
...
#define DOUBLE_EQUALITY_EPSILON 1e-5
...
typedef struct 
{
   UCHAR labelType;       // one of STRING_LABEL or NUMERIC_LABEL or VECTOR_LABEL
   union 
   {
      char *stringLabel;
      double numericLabel;
      struct v_struct{
    	  double * vectorLabel;
    	  char *stringLabel;
    	  UCHAR vectComparisonType;	// Vector comparison function to be used.
    	  int numberOfDimensions;		// Number of dimensions for vector label.

    	  // The two vars sum and stdev will be used to shorten matching time of equality of two vector labels.
    	  double mean;				// Sum of all elements in the vector label.
    	  double stdev;				// Standard deviation of all elements in the vector label
      } vStruct;
   } labelValue;
...
typedef struct 
{
   ...
   ULONG discoveryLevel; // 1 -> no discovery after pre-defined sub discovery
   	   	   	   	   	   	 // 2 -> discovery of only pre-defined subs and their extensions
   BOOLEAN forceDiversity;	// if TRUE, SUBDUE is forced to find diverse substructures.
   	   	   	   	   	     // 3 -> full discovery (default)
   ...
   ULONG fileOutputLevel; // More file output as value increases
   ...
   BOOLEAN compressWithAll; // If TRUE, graph is compressed with all
   	   	   	   	   	   	   //subs discovered in previous layer.
...
// Global Variables
//---------------------------------------------------------------------------
extern VectorLimitEntry * vectorLimits;	// vectorLimits will hold tuples of the form
						// <name, numberOfDimensions, distType, maxDistance>.
						// maxDistance is the maximum distance between two
						// vector labels of size "numberOfDimensions" and using
						// distType as the distance function type.
						// It is volatile, and re-calculated for each graph.
extern int vectorLimitsEntryCount;
...
BOOLEAN GraphMatch(Graph *, Graph *, LabelList *, double, double *,
                   VertexMap *, BOOLEAN);
double InexactGraphMatch(Graph *, Graph *, LabelList *, double,
				   VertexMap *, BOOLEAN);
...
void PrintVertex(Graph *, ULONG, ULONG, LabelList *, FILE *);
void PrintEdge(Graph *, ULONG, ULONG, LabelList *, FILE *);
...
void PrintLabel(ULONG, LabelList *, FILE *);
...
InstanceList *FindSingleVertexInstances(Graph *, Vertex *, Parameters *, double);
InstanceList *ExtendInstancesByEdge(InstanceList *, Graph *, Edge *,
                                    Graph *, Parameters *, double);
double EdgesMatch(Graph *, Edge *, Graph *, Edge *, Parameters *, double);
...
void SubListInsert(Substructure *, SubList *, ULONG, BOOLEAN, LabelList *, Parameters *);
BOOLEAN MemberOfSubList(Substructure *, SubList *, LabelList *);
void FreeSubList(SubList *);
void PrintSubList(SubList *, Parameters *);
void PrintSubListToFile(SubList *, Parameters *);
...
void PrintInstanceToFile(Instance *, ULONG, Graph *, LabelList *, FILE *);
...
void PrintInstanceListToFile(InstanceList *, Graph *, LabelList *, FILE *, double);
