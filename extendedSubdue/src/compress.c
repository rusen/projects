...
void CompressWithPredefinedSubs(Parameters *parameters)
{
   ...
   for (i = 0; i < numPreSubs; i++) 
   {
	  ...
	  // Estimate threshold for matching this subgraph to its instances.
	  threshold = parameters->threshold * (preSubs[i]->numVertices + preSubs[i]->numEdges);
      ...
      if ((numPosInstances > 0) || (numNegInstances > 0)) 
      {
         ...
         // Printing the sub's instances to file, if needed.
		 if (parameters->outputToFile && parameters->fileOutputLevel > 1)
		 {

			 // Create substructure label and print to file.
			if (parameters->incremental)
			{
				  sprintf(subLabelString, "%s_%s_%lu_%lu", PREDEFINED_PREFIX,
			...
			fprintf(outputFile, "Score: %lf\n", sub->value);

			// Print substructure definition to the file.
			fprintf(outputFile, "\nDefinition:\n");
			WriteGraphToFile(outputFile, preSubs[i], parameters->labelList,
							   0, 0, preSubs[i]->numVertices, FALSE);
			fprintf(outputFile, "\n");

			// Print instances to the file.
			 if (posInstanceList != NULL)
				 PrintInstanceListToFile(posInstanceList, posGraph,
						 parameters->labelList, outputFile, threshold);
			 if (negInstanceList != NULL)
				 PrintInstanceListToFile(negInstanceList, negGraph,
						 parameters->labelList, outputFile, threshold);
		 }
