   ...
   while (refInstanceListNode != NULL)
   {
      ...
      while (!empty)
      {
         ...
         while (instanceListNode != NULL)
         {
	        ...
	        parameters->labelList, 0.0, NULL, NULL, FALSE))
            {
                               ...
                               Parameters *parameters) 
{
   ...
   if (!found)
   {
      ...
      if (!overlapInOldIncrements)
      {
         ...
         SubListInsert(localSub, subList, 0, FALSE, parameters->labelList, parameters);
         instanceAdded = TRUE;
      }
   }
   if (instanceAdded)
...
BOOLEAN CheckForSubgraph(Graph *g1, Graph *g2, Parameters *parameters)
{
   ...
   double threshold = (g1->numEdges + g1->numVertices) * parameters->threshold;
